...
exports.addSolution = function(problemId, title, author, link, readTimes) {
...
exports.querySolutions = function(problemId) {
  return zrange('solutions:' + problemId, 0, -1).then(function(solutionIds) {
    var multi = client.multi();
    for(var i = 0, _length = solutionIds; i < _length; i++) {
      multi.hgetall(solutionIds[i]);
    }
    var exec = Q.nbind(multi.exec, multi);
    return exec();
  });
